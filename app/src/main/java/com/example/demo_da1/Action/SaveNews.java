...
public class SaveNews extends AppCompatActivity {
    int pos;
    String url="https://docbao.conveyor.cloud/api/tindaluus";
    ArrayList<News> arr=new ArrayList<>();
    ArrayList<News> arrtk;
    EditText edttk;
    Adapter adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.layout_savenews);
        ...
        adapter = new Adapter(SaveNews.this,arr);
        recyclerView.setAdapter(adapter);
        recyclerView.setLayoutManager(new LinearLayoutManager(SaveNews.this));
        recyclerViewTK.setLayoutManager(new LinearLayoutManager(SaveNews.this));
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                ...
                if (!d.equals("")) {
                    for (int i = 0; i < arr.size(); i++) {
                        News tt = arr.get(i);
                        String ten = tt.getTitle().toUpperCase().trim();
                        if (ten.contains(d)) {
                            arrtk.add(tt);
                        }
                    }
                    ...
                    if (arrtk.size() > 0) {
                        adapter = new Adapter(SaveNews.this, arrtk);
    ...
    public void Xoa(){
        APIService.apiService.sendDeleteLuu(arr.get(pos).getTitle()).enqueue(new Callback<Void>() {
            @Override
            public void onResponse(Call<Void> call, Response<Void> response) {
                new MyTask().execute(url);
                Toast.makeText(SaveNews.this, "Đã xóa khỏi yêu thích", Toast.LENGTH_SHORT).show();
            }

            @Override
            public void onFailure(Call<Void> call, Throwable t) {
                Toast.makeText(SaveNews.this, "Lỗi", Toast.LENGTH_SHORT).show();
            }
        });
    }
...
//            adapter=new CustemAdapter(SaveNews.this,R.layout.row_list_news,arr);
//            luutt.setAdapter(adapter);
